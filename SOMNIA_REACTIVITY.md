# Somnia Reactivity: Teknik Entegrasyon Rehberi (PulsePastures)

Bu döküman, PulsePastures projesinin Somnia Network üzerindeki native reactivity özelliklerini nasıl kullandığını ve en son DevRel standartlarına (v2.0) uygun en iyi uygulama dökümantasyonudur.

## 1. Ağ Bilgileri (Somnia Testnet)
...
- **Explorer**: [SomniaScan](https://shannon-explorer.somnia.network)
...
- **Handler**: `SomniaEventHandler` temel sınıfı kullanılarak güvenli bir şekilde olaylar işlenir.
- **Gereksinim**: Kontrat sahibinin cüzdanında en az **32 STT** bulunmalıdır.

## 3. Gaz ve Ücret Yapılandırması (Kritik)

Somnia Reactivity'nin sağlıklı çalışması için şu gaz parametreleri "gwei" cinsinden tanımlanmıştır:
...
---
*Bu proje Somnia DevRel standartlarına göre modernize edilmiştir.*
